...
namespace MGF::Render {
	...
	{
		if (name = SelectParam(name, altName))
		{
	...
	QVector3D MGFMaterialFileReader::ReadVector3(const char* name, const char* altName)
	{
		if (name = SelectParam(name, altName))
		{
			return IMaterialParamReader::StrToVector(ConfigVariables[name]);
		}
	...
	QColor MGFMaterialFileReader::ReadColour(const char* name, const char* altName)
	{
		if (name = SelectParam(name, altName))
		{
			return IMaterialParamReader::StringToColor(ConfigVariables[name]);
		}
	...
	{
		return a && ConfigVariables.contains(a) ? a	: b && ConfigVariables.contains(b) ? b : nullptr;
	}
