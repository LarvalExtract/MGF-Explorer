...
namespace MGF::Render
{
	Qt3DRender::QMaterial* MaterialFactory::CreateMaterial(IMaterialParamReader& paramReader)
	{
		...
		QFilterKey* forwardRenderingStyle = new QFilterKey(material);
		forwardRenderingStyle->setName(QStringLiteral("renderingStyle"));
		forwardRenderingStyle->setValue(QStringLiteral("forward"));
		technique->addFilterKey(forwardRenderingStyle);

		QEffect* effect = new QEffect;
		effect->setParent(material);

		// Unlit
		effect->addParameter(new QParameter("bEnableLights", true));
