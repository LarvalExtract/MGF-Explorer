...
Qt3DRender::QGeometryRenderer* MA::MeshLibrary::CreateMesh(const MGFFile& meshFile, const MGFFile& sourceFile)
{
    ...
    const MGFFile* verticesFile = meshFile.FindRelativeItem(verticesFilename);
    const MGFFile* indicesFile = meshFile.FindRelativeItem(facesFilename);
...
Qt3DRender::QGeometryRenderer* MA::MeshLibrary::CreateMesh(const MGFFile& verticesFile, const MGFFile& indicesFile, const std::string_view material, const MGFFile& sourceFile, Qt3DRender::QGeometryRenderer::PrimitiveType primitiveType)
{
    const uint64_t key =
...
int MA::MeshLibrary::LoadVertexBuffer(const MGFFile& verticesFile, Qt3DCore::QGeometry& geom, QVector3D& minExtents, QVector3D& maxExtents)
{
	...
	if (MGFFileDeserializer deserializer(verticesFile); verticesFile.FilePath().extension() != ".geomvert")
	{
...
void MA::MeshLibrary::LoadIndexBuffer(const MGFFile& indicesFile, Qt3DCore::QGeometry& geom)
{
    ...
    if (MGFFileDeserializer deserializer(indicesFile); indicesFile.FilePath().extension() != ".geomface")
    {
...
void MA::MeshLibrary::SetGeometryAttributesFromFlags(uint32_t flags, uint32_t count, uint32_t stride, Qt3DCore::QGeometry& geom, Qt3DCore::QBuffer* vertexBuffer)
{
    ...
    case 0x00000101:    // 16 bytes
        ...
        geom.addAttribute(new QAttribute(vertexBuffer, attrPackedNormal, QAttribute::VertexBaseType::UnsignedInt, 1, 12, stride));
        break;
