    ...
    : QWidget(parent)
{
    ...
    {   
        ...
        Viewport->setNormalizedRect(QRectF(0.0f, 0.0f, 1.0f, 1.0f));
        {
            ...
            {
                FrustumCulling = new Qt3DRender::QFrustumCulling(ClearBuffers);
                {
                    //TechniqueFilter = new Qt3DRender::QTechniqueFilter(FrustumCulling);
                    {
                        Camera = new Qt3DRender::QCamera(FrustumCulling);
    ...
    FrameLayout(frameLayout)
{
    ...
    CameraControllerEntity->setLinearSpeed(100.0f);

   //QObject::connect(UpdateCameraMovementSpeedAction, &Qt3DLogic::QFrameAction::triggered, [this](float dt) {
   //    CameraControllerEntity->setLinearSpeed(UpdateCameraMovementSpeedAction->scroll)
   //});
   //
   //UpdateCameraMovementSpeedAction->connect(d->m_frameAction, &Qt3DLogic::QFrameAction::triggered,
   //    this, [this](float dt) {
   ...
   //        moveCamera(state, dt);
   //    });
   //CameraControllerEntity->addComponent(UpdateCameraMovementSpeedAction);
...
{
     //const float speed = CameraControllerEntity->linearSpeed();
     //CameraControllerEntity->setLinearSpeed(speed + speed * e->angleDelta().y());
}
