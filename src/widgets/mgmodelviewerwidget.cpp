...
#include "ui_mgmodelviewerwidget.h"

#include "mgfresourcemanager.h"

static int s_Count = 0;

MGModelViewerWidget::MGModelViewerWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MGModelViewerWidget)
{
    m_SupportedTypes.insert(MGFFileType::Model);
    m_SupportedTypes.insert(MGFFileType::Node);
...
MGModelViewerWidget::~MGModelViewerWidget()
{
...
void MGModelViewerWidget::LoadMGFItem(const MGFTreeItem *item)
{
    ...
    auto& model = ResourceManager.GetModel(*item, m_SceneManager);
...
void MGModelViewerWidget::InitialiseScene()
{
...
bool MGModelViewerWidget::eventFilter(QObject *watched, QEvent *event)
{
    if (!isVisible())
        return false;
...
void MGModelViewerWidget::UpdateFrame(QTimerEvent* timerEvt)
{
    if (m_bMouseHeld)
    {
...
void MGModelViewerWidget::ResetCamera(const Ogre::AxisAlignedBox &modelAABB)
{
...
void MGModelViewerWidget::UpdateCamera()
{
    auto c = cursor();
...
void MGModelViewerWidget::UpdateCameraAxes(float yaw, float pitch)
{
...
void MGModelViewerWidget::UpdateKeyState(int key, bool state)
{
    switch (key)
    {
...
void MGModelViewerWidget::UpdateTabs(const MGNode& selectedModel)
{
